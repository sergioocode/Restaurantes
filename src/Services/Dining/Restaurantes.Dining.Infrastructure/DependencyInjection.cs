...
using Restaurantes.Dining.Infrastructure.Persistence.Stores;
...
public static class DependencyInjection
{
    ...
    )
    {
        ...
        services.AddHttpClient("payments", client =>
            client.BaseAddress = new Uri(configuration["Payments:BaseAddress"]
                ?? throw new InvalidOperationException("Payments:BaseAddress is required.")));
        return services;
    }

    public static async Task MigrateDiningDatabaseAsync(this IServiceProvider services, CancellationToken ct = default)
    {
