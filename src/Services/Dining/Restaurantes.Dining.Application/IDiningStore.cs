...
public interface IDiningStore
{
    ...
    Task<Dictionary<Guid, DiningSession>> ReadActiveSessionsAsync(Guid restaurantId, CancellationToken ct);
    ...
    Task<DiningSession?> ReadValidatedSessionAsync(Guid sessionId, Guid restaurantId, Guid tableId, CancellationToken ct);
    ...
    Task<DiningPendingPayment?> FindPendingPaymentAsync(Guid orderId, CancellationToken ct = default);
...
public enum DiningStoreFailure { Duplicate, ForeignKey, Concurrency }
