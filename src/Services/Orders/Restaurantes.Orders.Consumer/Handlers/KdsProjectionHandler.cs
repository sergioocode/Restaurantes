...
public sealed class KdsProjectionHandler(OrderReadDbContext dbContext, TimeProvider timeProvider)
{
    public async Task<KdsProjectionNotification?> ProjectAsync(
        Guid messageId,
        string? type,
    ...
    {
        ProjectionData data = Deserialize(type, body.Span);
        bool shouldNotify = false;
        if (
            !await dbContext.InboxMessages.AnyAsync(item => item.Id == messageId, cancellationToken)
        )
        {
